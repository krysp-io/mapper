...
To make it easier for a standard third party integration, a [public mapper definition](https://github.com/krysp-io/mapper-definitions) repository is available which allows developers to select the required mapping for a JSON transformation.
...
link: [Public mapper definition repository](https://github.com/krysp-io/mapper-definitions)
...
<ol>
<li>Add an Inject node with below sample JSON, You can add below data in Inject node properties by double clicking the inject node and changing the <code>msg.payload</code> type to JSON
...
<li>Add Mapper node to the flow and edit the properties of mapper node by double clicking the mapper node</li>

<li>You can create a new mapping by clicking on edit button on Mapper properties panel and having <strong>Add new Mappings...</strong> selected.
...
- You can click on <strong>Load</strong> button to load JSON to schema field and edit if required.
...
<li><strong>(Optional)</strong> You can select <strong>Clean up</strong> checkbox to remove un-mapped fields.</li>
...
<li><strong>Set Field Values</strong>
<br>
...
<li>Attach the mapper node to the inject node.</li>
<li>Attach debug node to mapper node</li>
<li>On executing this flow, the expected output will be as follows,</li>
